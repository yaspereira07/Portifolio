...
formContato.addEventListener('submit', async function(event) {
    event.preventDefault(); // Impede que o site vá para a página branca do Formspree
    ...
    // Muda o texto do botão enquanto envia
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Pega o idioma atual para dar o alerta na língua certa
            const currentLang = document.getElementById("current-lang").innerText.toLowerCase();
            if(currentLang === 'pt') alert("✅ Mensagem enviada com sucesso!");
            else if (currentLang === 'en') alert("✅ Message sent successfully!");
            else alert("✅ ¡Mensaje enviado con éxito!");

            // Esconde o aviso roxo de caracteres
